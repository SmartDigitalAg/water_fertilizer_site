document.addEventListener('DOMContentLoaded', function() {
  ...
  // ========== 사전 계산된 데이터 캐시 ==========
  let precomputedDataCache = {}; // { scenario: Map }
  let precomputedDataLoaded = {}; // { scenario: boolean }
  ...
  const climateNames = {
    'rhum': '상대습도 (rhum)', 'rsds': '일사량 (rsds)',
    'tmax': '최고기온 (tmax)', 'tmin': '최저기온 (tmin)', 'wspd': '풍속 (wspd)'
  };

  const stageNames = {
    '유묘기': '유묘기', '분얼기': '분얼기', '생육재생기': '생육재생기',
    '분얼및신장기': '분얼 및 신장기', '출수및등숙기': '출수 및 등숙기'
  };
  ...
  function createLegend(min, max) {
    ...
    // 데이터 없음을 맨 위에 배치
    const labels = [{
      color: 'rgba(255, 255, 255, 1)', // 흰색 (불투명)
      label: '데이터 없음'
    }];
  ...
  /**
   * 정적 지도 시각화 (2071-2100년 평균 + 작물별 범례)
   */
  async function visualizeStaticMaps(crop) {
    const scenarios = ['ssp126', 'ssp245', 'ssp370', 'ssp585'];
    const mapIds = ['map1', 'map2', 'map3', 'map4'];
    const legend = getCurrentLegend();

    console.log(`🗺️ 정적 지도 시각화: ${crop}, 21세기 말 (2071-2100년 평균)`);
    ...
    for (let i = 0; i < scenarios.length; i++) {
      ...
      dataMap.forEach((row) => {
        if (row.crop === crop && row.year >= 2071 && row.year <= 2100) {
    ...
    console.log('✅ 정적 지도 시각화 완료 (21세기 말, 2071-2100년)');
  }
  ...
  document.getElementById('cropType').addEventListener('change', async function() {
    ...
    await Promise.all([
      ...
      visualizeStaticMaps(currentCrop),
  ...
  document.querySelectorAll('.category_btn_group button').forEach(button => {
    button.addEventListener('click', async function() {
      document.querySelectorAll('.category_btn_group button').forEach(btn =>
    ...
    try {
      ...
      const map = new ol.Map({
        ...
        view: new ol.View({
          ...
          zoom,
          minZoom: interactive ? 5.5 : zoom,
          maxZoom: interactive ? 10 : zoom
        }),
  ...
  stationNames.forEach((name, index) => {
    stationSelect.insertAdjacentHTML('beforeend', `<option value="${index + 1}">${name}</option>`);
  });
      ...
      dataTableBody.insertAdjacentHTML('beforeend', `
        <tr style="background:#f9f9f9;">
          <td colspan="2" style="font-weight:600; padding:8px; border-top:2px solid #ddd;">
            ${stageNames[stageName]} (${stageData.dateRange})
          </td>
        </tr>
