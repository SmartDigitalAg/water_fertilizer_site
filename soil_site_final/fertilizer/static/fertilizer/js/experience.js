...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ========== 1. 면적 단위 변환 기능 추가 ==========
    let currentUnit = '㎡'; // 초기값: 제곱미터
    const areaRadios = document.querySelectorAll('input[name="area_gb"]');

    // 라디오 버튼에 value 값 설정
    areaRadios[0].value = '㎡';
    areaRadios[1].value = '평';

    // 단위 변경시 면적값 자동 변환
    areaRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            const val = parseFloat(elements.area.value);
            if (val && !isNaN(val) && currentUnit !== this.value) {
                // m2 → 평: /3.3058, 평 → m2: *3.3058
                const converted = currentUnit === '㎡' ? val / 3.3058 : val * 3.3058;
                elements.area.value = converted.toFixed(2);
        ...
        radio.addEventListener('change', function() {
            if (this.value === '유기농(논)') {
                ...
                if (elements.cropType.value !== '00') {
                    alert('유기농(논)은 곡류(벼) 작물을 선택하셔야 처방하실 수 있습니다.');
                    // 논으로 되돌림
                    document.querySelector('input[name="field_type"][value="논"]').checked = true;
                }
            }
        });
    });
    ...
    elements.cropType.addEventListener('change', async function() {
        ...
        // 곡류(벼)가 아닌 작물을 선택했는데 유기농(논)이 선택되어 있으면 논으로 변경
        if (this.value !== '00') {
            const organicRadio = document.querySelector('input[name="field_type"][value="유기농(논)"]');
            if (organicRadio && organicRadio.checked) {
                document.querySelector('input[name="field_type"][value="논"]').checked = true;
            }
        }
    });
    ...
    async function calculateFertilizer(stage, n, p, k, qy, baseData) {
        ...
        const area2 = areaUnit === '㎡' ? areaValue / 3.3058 : areaValue;
