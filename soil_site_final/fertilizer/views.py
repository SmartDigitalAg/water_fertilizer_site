...
def get_crop_list(crop_type_code, mode="NOTCROPNOCODE"):
    """작물 목록 조회 - mode 파라미터로 API 모드 지정"""
    params = {
        "mode": mode,  # NOTCROPNOCODE(비료사용처방/체험) 또는 STADCROP(표준사용량)
        "code": crop_type_code,
        "_": int(time.time() * 1000)
    }
    response = api_request("https://soil.rda.go.kr/cmm/common/ajaxCall.do", params=params)
...
def search_crops_by_keyword(keyword, mode="NOTCROPNOCODE"):
    """작물명 키워드 검색 - mode 파라미터 추가"""
    ...
    for crop_type_code in crop_types:
        crops = get_crop_list(crop_type_code, mode)  # mode 파라미터 전달
...
def prescription_api(request):
    ...
    try:
        ...
        # 작물 목록 조회
            ...
            result = get_crop_list(crop_type, mode="NOTCROPNOCODE")
            return JsonResponse({'success': True, 'data': result})
        ...
        # 작물명 키워드 검색
            ...
            result = search_crops_by_keyword(keyword, mode="NOTCROPNOCODE")
            return JsonResponse({'success': True, 'data': result})
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'get_crops':
                ...
                result = get_crop_list(crop_type, mode="STADCROP")
                return JsonResponse({'success': True, 'data': result})
            ...
            if result['success']:
                # 작물명 동적 조회
                crop_name = '알 수 없는 작물'
                crop_type_code = crop_code[:2]
                crops_list = get_crop_list(crop_type_code, mode="STADCROP")
                crop_name = crops_list.get(crop_code, '알 수 없는 작물')
