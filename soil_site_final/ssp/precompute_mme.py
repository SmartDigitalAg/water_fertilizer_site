...
DATA_PATH = Path(r'C:\code\soil_site_ksafm\ssp\MME')
OUTPUT_PATH = Path(r'C:\code\soil_site_ksafm\ssp\water_req_mme')  # 변경된 출력 경로
...
def read_climate_data(scenario, year, climate_element, target_dates):
    ...
    file_path = DATA_PATH / f'SQM_MME_{climate_element}_{scenario}_{year}.csv'
    ...
    try:
        ...
        for target_date in target_dates:
            # MM-DD 형식으로 매칭 (예: 11-05)
            matched = df[df['date'].str.contains(f'-{target_date}$', na=False, regex=True)]

            if not matched.empty:
                row = matched.iloc[0]
                station_data = {}

                # date 컬럼을 제외한 나머지 컬럼들 (지점 ID)
                    ...
                    if col != 'date':
                        try:
                            ...
                            if pd.notna(val):  # NaN이 아닌 경우만
                                station_data[col] = val
                        except:
                            pass

                if station_data:  # 데이터가 있는 경우만 추가
    ...
    for stage_name, stage_info in stages.items():
        ...
        for date in date_range:
            ...
            for station_id in station_ids:
                rhum = climate_data_by_element['rhum'].get(date, {}).get(station_id)
...
def precompute_scenario(scenario):
    ...
    if len(results) == 0:
        print(f"⚠️ {scenario} 시나리오에 처리할 데이터가 없습니다.")
        return
    ...
    # 비압축 CSV 저장 (임시)
    csv_path = OUTPUT_PATH / f'water_req_MME_{scenario}.csv'
    df.to_csv(csv_path, index=False, encoding='utf-8-sig')

    print(f"CSV 파일 크기: {csv_path.stat().st_size / 1024 / 1024:.2f} MB")

    # gzip 압축
    gz_path = OUTPUT_PATH / f'water_req_MME_{scenario}.csv.gz'
