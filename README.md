...
- **주요기능**: 
  ...
  - 생육단계별 물필요량 표출 (흙토람 크롤링-기존 18개 작물 유형)
...
- **주요기능**: 
  ...
  - 167개 지점 KACE-1-0-G 모델의 기상 데이터 기반 FAO Penman-Monteith 기준증발산량 산정
...
### 4. 주소 검색
- V-World 검색 2.0 API
- 검색 제한: 300건 (초과 시 상세 입력 요청하도록 설정)
- 페이지당 최대 1,000개, 최대 10페이지 조회(필요 시 설정 변경으로 10,000건까지 확장 가능)
- 래퍼런스 : https://www.vworld.kr/dev/v4dv_search2_s001.do 
...
**서버 관리 권한**: AWS EC2 및 https(Cloudflare)는 교수님 계정으로 관리되므로 문제 발생 시 자체 해결 불가, 교수님께 말씀드리기
